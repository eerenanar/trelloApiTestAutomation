...
public class APIVerification extends ExtentReportListener {
    ...
    public static void responseCodeValidation(Response response,int statusCode){
        ...
        try {
            ...
            test.log(LogStatus.PASS,"Succesfully validated status code, status code is :"+response.getStatusCode());

        } catch (AssertionError e) {
            test.log(LogStatus.FAIL,e.fillInStackTrace());
            test.log(LogStatus.FAIL,"Expected status code is "+statusCode+"but actual code is"+response.getStatusCode());
